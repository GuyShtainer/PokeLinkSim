...
static bool setup_save(int s, const BrEntry* e, const char* path, Gen3Version ver) {
  ...
  /* Pre-fill the selection from this player's saved team preset (matched by
   * species + nickname, exactly as QUICK MODE does in omit_from_prefs) so
   * previously-omitted party mons start greyed-out instead of everything
   * defaulting to chosen. With no saved preset tp_apply keeps chk[]=true, so the
   * whole party is selected -- the original first-time behaviour. */
  {
    Gen3DisplayParty dp;
...
static void run_mix_flow(void) {
  ...
  if (cst == SF_OK) {
    /* Persist each side's party selection so it pre-fills (greyscale) next time
     * and QUICK MODE uses the real choices. Best-effort: a failed write just
     * isn't remembered and never blocks the completed mix. */
