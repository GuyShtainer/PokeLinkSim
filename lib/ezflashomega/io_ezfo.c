...
// Soft-reboot to the EZ-Flash kernel/menu. Quiesce the system first (a bare
// SetRompage+SoftReset can leave an in-flight DMA/timer running, and the gamepak
// window is unmapped the instant the rompage flips), then map the bootloader and
// SoftReset into it. Must be EWRAM_CODE: ROM is gone after the rompage swap, so
// this function and its for(;;) tail must already be resident in EWRAM.
void EWRAM_CODE _EZFO_reboot(void) {
  ...
  *(vu16*)(REG_BASE_ + 0x010E) = 0;
  ...
  asm volatile("swi 0x00" ::: "memory");  // BIOS SoftReset -> 0x08000000 (kernel/menu)
  for (;;) {}                             // unreachable
}
